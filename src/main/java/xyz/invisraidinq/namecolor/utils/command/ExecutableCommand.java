...
public class ExecutableCommand implements CommandExecutor, TabCompleter {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args){
        ...
        if(args.length < 2) {
            for (CommandArgument argument : arguments) {
                String permission = argument.getPermission((PluginCommand) command);
                if (permission == null || sender.hasPermission(permission)) results.add(argument.getName());
            }
            if(results.isEmpty()) return null;
        } else {
            CommandArgument argument = getArgument(args[0]);
            if(argument == null) return results;
            String permission = argument.getPermission((PluginCommand) command);
    ...
    private List<String> getCompletions(String[] arguments, List<String> input, int limit) {
